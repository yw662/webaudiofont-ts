export class loader {
    player: { adjustPreset: (context: AudioContext, preset: { zones: [] }) => void; };
    cache: { path: string, name: string }[];
    ...
    startLoad(context: AudioContext, path: string, name: string) {
        if (window[name]) {
            return;
        }
        if (this.cache.some(c => c.name === name)) {
            return;
        }
        this.cache.push({ path, name });
        const r = document.createElement('script');
        r.setAttribute('type', 'text/javascript');
        r.setAttribute('src', path);
        document.getElementsByTagName('head')[0].appendChild(r);
        this.decodeAfterLoading(context, name);

    }
    decodeAfterLoading(context: AudioContext, name: string) {
        this.waitOrFinish(name, () => {
            this.player.adjustPreset(context, window[name]);
        })
    }
    waitOrFinish(name: string, cb: () => void) {
        if (window[name]) setTimeout(cb, 0);
        else setTimeout(() => this.waitOrFinish(name, cb), 111);// wtf is this ?
    }
    loaded(name: string) {
        if (!window[name]) return false;
        if ((window[name].zones as any[]).some(zone => !zone.buffer)) return false;
        return true;
    }
    progress() {
        return this.cache.every(item => this.loaded(item.name));
    }
    waitLoad(cb: () => void) {
        if (this.progress()) setTimeout(cb, 0);
        else setTimeout(() => this.waitLoad(cb), 333);// wtf again is this ?
    }
    instrumentTitles() {
    ...
    instrumentInfo(n: number) {
        const key = this.instrumentKeyArray[n];
        const p = Number.parseInt(key.substr(0, 3));
        return {
            variable: '_tone_' + key,
    ...
    findInstrument(n: number) {
        return Math.max(0, this.instrumentKeyArray.findIndex(v => Number.parseInt(v.substring(0, 3)) === n));
    }
    drumTitles() {
